		...
		int *tmp_fd_in)
{
	...
	res = 0;
	if (curr_token->next && (curr_token->type == REDIRECT_IN || curr_token->type == HEREDOC))
		res = set_redirect_in(ms, curr_token, tmp_fd_in);
	if (curr_token->next && (curr_token->type == REDIRECT_OUT || curr_token->type == APPEND))
		...
		size_t cmd_size)
{
	...
	if (tmp_fd_in > -1)
		last_cmd->fd_in = tmp_fd_in;
	if (tmp_fd_out > -1)
		last_cmd->fd_out = tmp_fd_out;
	return (1);
}
